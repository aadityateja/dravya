...
updateStopTransition() {
  ...
  if (this.revealStarted) {
    ...
    updateEventLabels() {
      ...
      const revealFade = 1 - smoothstep(0.9, 0.955, this.progress);
      let activeIndex = -1;
      let activeValue = 0;
    ...
    updateOverlay() {
      ...
      const introExit = smoothstep(0.028, 0.108, this.progress);
      intro.style.opacity = String(1 - introExit);
      intro.style.transform = `translate3d(0, ${(-20 * introExit).toFixed(2)}px, 0)`;

      const cornerOpacity =
        smoothstep(0.075, 0.18, this.progress) * (1 - smoothstep(0.965, 0.995, this.progress) * 0.35);
